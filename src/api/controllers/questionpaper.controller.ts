...
async function generateQuestionPaperAndStore(queryObject: { class_range: any; boards: any; subjects: any; total_number_of_questions: any; difficulty: any; question_paper_language: any; topic: any; questionType: any; }, userUuid: any) {
    ...
    try {
        ...
        const questionPromises = questionsData.map(async (qData: { options: any[]; subject: any; class: any; board: any; question: any; image: any; correctAnswer: string; }) => {
...
const evaluateQuestionPaper = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (nonMcqQuestions.length > 0) {
            const nonMcqResults = await evaluateNonMCQ(
                nonMcqQuestions,
                questionPaper.studentAnswers
            );
...
function evaluateMCQ(studentAnswers: any[], correctAnswers: any[]) {
    ...
    return studentAnswers.every((ans: any) => correctAnswers.includes(ans)) &&
...
async function evaluateNonMCQ(questions: any[], studentAnswers: any[]) {
    try {
        ...
        const evaluatedAnswers = response.data.evaluations.map((evalu: { maxMarks: number; marksObtained: number; questionUuid: any; feedback: any; }) => {
            ...
            return {
                ...
                answer: studentAnswers.find(
                  (                        ans: { questionUuid: any; }) => ans.questionUuid === evalu.questionUuid
                ).answer,
